...
class Local_Sync_Admin {
	...
	public function update_1_0_1_handle() {
		...
		if( empty($local_sync_version_in_options_table) 
			...
			$this->app_functions->prepare_and_set_prod_key();
			$this->local_sync_options->set_option('local_sync_version', '1.0.1');
		}
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/local-sync-admin.js', array( 'jquery' ), $this->version, false );

		wp_enqueue_script('local-sync-jquery-ui-custom-js', plugins_url() . '/' . LOCAL_SYNC_PLUGIN_NAME . '/treeView/jquery-ui.custom.js',   array(), LOCAL_SYNC_VERSION);
	...
	public function modified_files_modal_ok() {
		...
		foreach ($user_selected_files as $key => $value) {
			if(stripos($key, 'wp-content/uploads/local_sync') === false && (!$value || $value == 'false') ){
				$un_selected_file_paths_str[] = '"' . $key . '"';
			}
		}
		$un_selected_file_paths_str = implode(',', $un_selected_file_paths_str);

		$chosen_action_local_sync = $_POST['chosen_action_local_sync'] ?? 'sync_from_live_site';
	...
	public function away_post_call($main_action, $url='', $sync_current_action='')	{
		...
		if($away_action_first_call){
			...
			$post_body['away_site_abspath'] = ABSPATH;
		...
		local_sync_die_with_json_encode(array(
			'error' =>  'Error No proper response from away post call'
		));
	}
	...
	public function start_zip_creation_dev()	{
		...
		@unlink(ABSPATH . "wp-content/uploads/local_sync/backups/local_sync_files-$current_sync_unique_id.zip");
	...
	public function save_settings_local_sync() {
		...
		if(!empty($_POST['data']['settings']['sync_type_db_or_files'])){
			$this->local_sync_options->set_option('sync_type_db_or_files', $_POST['data']['settings']['sync_type_db_or_files']);
		}
