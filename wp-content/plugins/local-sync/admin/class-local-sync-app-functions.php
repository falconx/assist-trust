...
class Local_Sync_App_Functions {
		...
		try{
			...
			do{
				...
				foreach ($files_to_zip as $kk => $file_obj) {
					...
					if($files_size_so_far > 3*1024*1024){
						local_sync_log($files_to_zip_this_time, "--------files_size_so_far--reached------");
					...
					if(is_local_sync_timeout_cut(false, 6)){
						$can_continue = false;

						local_sync_log('', "--------breaking zip creation--------");
			...
			local_sync_log($zip->last_error, "--------below while loop--------");
	...
	public function do_zip_download() {
		$current_sync_unique_id = $this->local_sync_options->get_option('current_sync_unique_id');
		$URL = rtrim($this->local_sync_options->get_option('prod_site_url'), '/') . "/wp-content/uploads/local_sync/backups/local_sync_files-$current_sync_unique_id.zip";
		$file = rtrim($this->local_sync_options->get_backup_dir(), '/') . '/' . "local_sync_files-$current_sync_unique_id.zip";
		...
		$bridge_fs_obj = new LocalSyncFileSystem();
		$download_result = $bridge_fs_obj->multi_call_download_using_curl($URL, $file, $downloadResponseHeaders, $prev_result);
	...
	public function do_zip_upload()	{
		$current_sync_unique_id = $this->local_sync_options->get_option('current_sync_unique_id');
		// $URL = rtrim($this->local_sync_options->get_option('prod_site_url'), '/') . '/wp-content/uploads/local_sync/backups/local_sync_files.zip';
		$URL = rtrim($this->local_sync_options->get_option('prod_site_url'), '/') . '/index.php';
		$file = rtrim($this->local_sync_options->get_backup_dir(), '/') . '/' . "local_sync_files-$current_sync_unique_id.zip";
	...
	public function process_download_local_file_list_dump()	{
		...
		if($this->local_sync_options->is_feature_valid('filesDiff')){
			...
			$current_sync_unique_id = $this->local_sync_options->get_option('current_sync_unique_id');
			$URL = rtrim($this->local_sync_options->get_option('prod_site_url'), '/') . "/wp-content/uploads/local_sync/backups/local_sync_file_list_dump-$current_sync_unique_id.sql.gz";
			$file = rtrim($this->local_sync_options->get_backup_dir(), '/') . '/' . "local_sync_file_list_dump-$current_sync_unique_id.sql.gz";
	...
	public function save_local_site_file_list_dump_file($file_data = null, $start_range = 0, $end_range = 0) {
		...
		if( $start_range == 0 && file_exists($file_list_dump_file) ){
			@unlink($file_list_dump_file);
	...
	public function save_full_zip_file($file_data = null, $start_range = 0, $end_range = 0) {
		...
		if( $start_range == 0 && file_exists($full_zip_file) ){
			// @unlink($full_zip_file);
