...
// --------------------------------------------------------------------------------
	...
	define( 'PCLZIP_OPT_IWP_EXCLUDE_EXT', 78998 );//darkCode
	...
	*/
	...
	$v_options[PCLZIP_OPT_IWP_EXCLUDE_EXT] = array();
		...
		if ($v_size > 1) {
			...
			if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
				...
				$v_result = $this->privParseOptions($v_arg_list, $v_size, $v_options,
													 ...
													 PCLZIP_OPT_IWP_EXCLUDE_EXT => 'optional',
																									 //, PCLZIP_OPT_CRYPT => 'optional'
													 ));
				if ($v_result != 1) {
					return 0;
...
//    $v_path = "./";
		...
		$v_options[PCLZIP_OPT_STOP_ON_ERROR] = FALSE;
		$v_options[PCLZIP_OPT_REPLACE_NEWER] = TRUE;
	$v_options[PCLZIP_OPT_CHUNK_BLOCK_SIZE] = 15*1024*1024*1024;
	$v_options[PCLZIP_OPT_FILE_EXCLUDE_SIZE] = 15*1024*1024*1024;
...
// --------------------------------------------------------------------------------
	...
	function privParseOptions(&$p_options_list, $p_size, &$v_result_list, $v_requested_options=false)
	{
		...
		case PCLZIP_OPT_IWP_EXCLUDE :
			...
			$i++;			  
			break;
		case PCLZIP_OPT_IWP_EXCLUDE_EXT :
			if (is_array($p_options_list[$i+1])) {
							$v_result_list[$p_options_list[$i]] = $p_options_list[$i+1];
			}
			$i++;			  
	...
	//$limitSize = 15*1024*1024*1024;
		...
		// ----- Look for real file or folder
			...
			{
				$exclude = isset($p_options[PCLZIP_OPT_IWP_EXCLUDE]) ? $p_options[PCLZIP_OPT_IWP_EXCLUDE] : array();
				$exclude_extensions = isset($p_options[PCLZIP_OPT_IWP_EXCLUDE_EXT]) ? $p_options[PCLZIP_OPT_IWP_EXCLUDE_EXT] : array();
				$skip_this = false;
				if(!empty($exclude)){
...
// TBC : Removed    $p_header['stored_filename'] = $v_stored_filename;
	...
	$p_header['stored_filename'] = $p_filename;
		...
		if (($v_file = @fopen($p_filedescr['filename'], "rb")) == 0) {
					PclZip::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL, "Unable to open file '$p_filename' in binary read mode. Please try changing the file permission to 644 or exclude this file from your backup.");
	...
	$next_extract_id = (empty($extract_multi_call_flags_losy['next_extract_id'])) ? 0 : $extract_multi_call_flags_losy['next_extract_id'];
	$v_pos_entry = (empty($extract_multi_call_flags_losy['v_pos_entry'])) ? $v_central_dir['offset'] : $extract_multi_call_flags_losy['v_pos_entry'];
	
	global $response_arr_local_sync;
	$response_arr_local_sync = array();
	// initialize_response_arr_local_syncay($response_arr_local_sync);
	...
	for ($i=$next_extract_id, $v_nb_extracted=$next_extract_id; $i<$v_central_dir['entries']; $i++)
		...
		$is_multicall_break = $this->check_for_ls_break();
		// if($i == 10){
		if($is_multicall_break){
			global $response_arr_local_sync;
			$response_arr_local_sync['next_extract_id'] = $i + 1;
			$response_arr_local_sync['is_extract_over'] = false;
			$response_arr_local_sync['status'] = 'partiallyCompleted';
			$response_arr_local_sync['break'] = true;
			$response_arr_local_sync['v_pos_entry'] = $v_pos_entry;
			$response_arr_local_sync['peak_mem_usage'] = (memory_get_peak_usage(true)/1024/1024);
		...
		$this->privCloseFd();
		$this->privSwapBackMagicQuotes();
		if( !empty($response_arr_local_sync['break']) ){
			//logExtractResponse($_REQUEST['extractParentHID'], array('status' => 'partiallyCompleted', 'sendResponse' => true, 'nextFunc' => 'backupFiles', 'responseParams' => $response_arr_local_sync));
			return $response_arr_local_sync;
		}
		// ----- Return
		return $v_result;
	}

	function check_for_ls_break(){
		...
		$extract_time_taken = time() - $local_sync_ajax_start_time;
		...
		if ($p_entry['status'] == 'ok') {
			...
			if (!(($p_entry['external']&0x00000010)==0x00000010))
			{
				...
				if ($p_entry['compression'] == 0) {
					...
					if ($GLOBALS['is_new_backup'] && $p_entry['filename'] = str_replace('wordpress/', '', $p_entry['filename'])){} 
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_EXTRACT].'(PCLZIP_CB_POST_EXTRACT, $v_local_header);');
	...
	function privDirCheck($p_dir, $p_is_dir=false)
	{
		...
		if ($GLOBALS['is_new_backup'] && $p_dir = str_replace('wordpress/', '', $p_dir)){} 
		if ($GLOBALS['needFileSystem']) {
			...
			if (!file_exists($p_dir)) {
				$result = mkdir($p_dir, 0755);
			}
		}

		if (!$result && !$GLOBALS['is_new_backup'])
		{
			// ----- Error log
